...
//
// Adapted from cparse by Jakob L�w
// 
// "THE BEER-WARE LICENSE":
// Jakob L�w <jakob@m4gnus.de> wrote this code. As long as you retain this notice you
...
let CParser = (function()
{
    ...
    function createParser()
    {
        ...
        // Sticky (lastIndex-anchored) version of a pattern.  Sticky regexes
        // scan the source in place at a given index without allocating a
        // substring copy, unlike the previous src.substr(index) + "^..."
        // approach used by peekSym/peekPat/lookaheadPat, which re-copied the
        // remainder of the source string on every call (quadratic behavior
        // on large inputs).  We cache one compiled sticky regex per distinct
        // pattern (by source+flags) so repeated calls with the same pattern
        // object don't recompile it.
        const stickyPatCache = new Map();
        function stickyPat(pat)
        {
            let key = pat.source + "\u0000" + pat.flags;
            let sticky = stickyPatCache.get(key);
            if (!sticky)
            {
                // strip a leading "^" (redundant with sticky mode) and
                // add the "y" flag if it's not already present
                let flags = pat.flags.indexOf("y") >= 0 ? pat.flags : pat.flags + "y";
                sticky = new RegExp(pat.source.replace(/^\^/, ""), flags);
                stickyPatCache.set(key, sticky);
            }
            return sticky;
        }
        const symPat = /(::|[_a-zA-Z][_a-zA-Z0-9]*)/y;
        ...
        let parse = function(src)
        {
            ...
            function parseInterface(kw)
            {
                ...
                let guidMatch = /^\{?([0-9a-f]{8}(?:-[0-9a-f]{4}){3}-[0-9a-f]{12})\}?$/i.exec(guid);
                if (!guidMatch)
                ...
                ifc.guid = guidMatch[1].toUpperCase();
            ...
            function peekSym()
            {
                symPat.lastIndex = index;
                let m = symPat.exec(src);
                return m ? m[1] : false;
            }

            function peekPat(pat)
            {
                let sticky = stickyPat(pat);
                sticky.lastIndex = index;
                let m = sticky.exec(src);
                return m ? m[0] : false;
            }
        ...
        function unparse(stmt)
        {
            ...
            {
                let s = (callingConvMap[t.callingConvention] || "C")
        ...
        let uuidof = function(type)
        {
            ...
            let ifcMatch = /^\s*interface\s+([a-z_][a-z0-9_]*)\s*$/i.exec(type);
            if (ifcMatch)
                type = ifcMatch[1];
