...
function trySprintf(...args)
{
    ...
    if (args.length > 1 && pat.test(args[0]))
    {
        ...
        let expansion = args[0].replace(pat, (match, flags, width, prec, spec) =>
        {
            ...
            case 's':
                return doString(a === null ? "null" : a === undefined ? "undefined" : a.toString());
...
this.console = {
    ...
    warning: function(...args) { this._log("warning", this.format(...args)); },
    ...
    _timerOp: function(label, remove)
    {
        ...
        if (t0 !== undefined)
        {
            this._log("log", label + ": " + (Date.now() - t0) + "ms");
            if (remove)
                delete this._timeTable[label];
        }
        else
            this._log("log", "Timer " + label + " does not exist");
    },
...
let { Event, EventTarget } = (() =>
{
    ...
    let EventTarget = class EventTarget
    {
        ...
        addEventListener(type, listener, options) {
            add(this, type, new Set(), listener, normalizeOptions(options));
        }
...
this.command =
{
    ...
    {
        let ranged = (a, b) => (id >= this[a] && id <= this[b] ? ({ name: a, index: id - this[a] }) : undefined);
...
this.MenuEvent = class MenuEvent extends Event
{
    ...
    addMenuItem(where, newItem, itemsArg)
    {
        ...
        let idx = arr.findIndex(matchFunc);
    ...
    deleteMenuItem(which, itemsArg)
    {
        ...
        case "object":
            if (RegExp.prototype.isPrototypeOf(which))
