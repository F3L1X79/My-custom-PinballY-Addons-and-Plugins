...
// which tables it holds and in which order, from the tables' play time and
// play count. Run with "node --test" from the project folder.
// ============================================================
...
function table(title, { playTime = 0, playCount = 0, isHidden = false, isConfigured = true } = {}) {
    return { id: title, title, playTime, playCount, isHidden, isConfigured };
}
...
test("ranks the tables from the most play time to the least", () => {
    ...
    assert.deepEqual(titles(getHallOfFame(tables)),
...
test("leaves out the tables never played, even when fewer than ten were played", () => {
    ...
    assert.deepEqual(titles(getHallOfFame(tables)), ["Medieval Madness"]);
});
